    ...
    if message.chat.id == AUTH_CHANNEL and join_db().isActive():
        ...
        if data.split("-", 1)[0] == "VJ":
            ...
            if vj and PREMIUM_AND_REFERAL_MODE == True:
                ...
                m=await client.send_sticker(chat_id = message.from_user.id, sticker = "CAACAgIAAxkBAAEBAAFwZ4tq3I31YhWmfRa1pUIldJcyqRIAAiAmAAJzs-FKip2fziZczzg2BA") 
        ...
        if not files_:
            ...
            try:
                ...
                title = '@Movie_DownIoad_bot  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
        ...
        files = files_
        title = '@Movie_DownIoad_bot  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files["file_name"].split()))
        ...
        if CUSTOM_FILE_CAPTION:
            try:
                ...
                f_caption=f_caption
        if f_caption is None:
            f_caption = f"@Movie_DownIoad_bot  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files['file_name'].split()))}"
        if not await db.has_premium_access(message.from_user.id):
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
